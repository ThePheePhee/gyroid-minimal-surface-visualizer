...
- Differential Diagnostics: shader-side finite-difference curvature color, directional hatching, minimality error, and focal-distance diagnostics applied directly to the raymarched surface.
...
- Bonnet / Strip Lab: approximate P/G/D field blending and strip coloration applied to the raymarched surface, with optional visible ribbon overlays.
- Labyrinth Skeleton: approximate distance-ridge point clouds for the selected complement labyrinth volume.
- Parallel / Focal Pointiness: approximate offset and near-caustic variants of the raymarched implicit field, plus focal highlighting.
- Screw Phase: approximate coordinate-domain screw defects that deform the raymarched surface, with optional minimality diagnostics.
...
- `src/math/surfaceTracing.ts` builds Developer Mode principal/asymptotic ribbon traces, approximate Bonnet/strip curves, and complement-labyrinth distance-ridge points. Curvature and focal diagnostics are handled by the GPU shader so they affect the visible surface.
...
- `src/rendering/raymarchMaterial.ts` contains the continuous GPU implicit renderer, complement-solid raymarch mode, and Developer Mode shader branches for screw deformation, approximate Bonnet/P-G-D blending, curvature diagnostics, strip coloration, and parallel/focal surface variants.
...
- Developer Bonnet/strip mode: approximate and visualization-only. It can replace the active field with a P-to-G-to-D blend and add coherent strip coloration, but it is not an exact Bonnet transformation.
...
- Developer focal/pointiness mode: approximate raymarched offset and near-caustic field variants based on finite-difference gradient and curvature proxies.
- Developer screw phase: approximate coordinate-domain phase defects that deform the rendered implicit field; minimality diagnostics are provided because this generally moves the surface away from exact minimality.
